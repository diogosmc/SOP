...
async def list_habits(
    ...
    return APIResponse(data=await service.list(user_id, pagination, is_active))
...
@router.post("/{habit_id}/log", response_model=APIResponse[HabitLogResponse])
async def log_habit(
    habit_id: uuid.UUID,
    data: HabitLogCreate,
    user_id: uuid.UUID = Depends(get_current_user_id),
    db: AsyncSession = Depends(get_db),
) -> APIResponse[HabitLogResponse]:
    service = HabitService(db)
    return APIResponse(data=await service.log_habit(habit_id, user_id, data))
